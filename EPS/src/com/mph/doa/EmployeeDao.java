...
public class EmployeeDao {
	...
	public void type_scroll_sensitive_rs_update()
	{
		...
		try 
		{
			ps = con.prepareStatement("select eid,ename from mphemp where eid = 5",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
			rs = ps.executeQuery();
	...
	public void batchUpdate()
	{
		...
		{
			st = con.createStatement();
			st.addBatch("update mphemp set ename ='TTT' where eid=1");
			st.addBatch("update mphemp set ename='KKK' where eid=4");
			st.addBatch("update mphemp set ename='FFF' where eid=3");
			st.addBatch("update mphemp set eid=5 where ename='SSS' ");
